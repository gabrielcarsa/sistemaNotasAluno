...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["nome"]))){
        ...
        $sql = "SELECT id FROM tb_aluno WHERE nome = :nome";
    ...
    if(empty(trim($_POST["avaliacao"]))){
        $avaliacao_err = "Por favor insira uma descrição.";     
    }else{
    ...
    if(empty($nome_err) && empty($avaliacao_err) && empty($nota_err) && empty($file_err)){//Validar se campos estão preenchidos
        if($_FILES['img_principal']['size'] > 0){//Valida se não está vazio 
            ...
            if($img_principal['size'] > 2097152){
                ...
                if($extensao != "jpg" && $extensao != "png"){
                    ...
                    $sql = "INSERT INTO tb_aluno (nome, avaliacao, nota_final, img_principal, data_cadastro) VALUES (:nome, :avaliacao, :nota_final, :img_principal, :data_cadastro)";
                    ...
                    if($stmt = $conn->prepare($sql)){
                        ...
                        $data_cadastro_aluno = date('Y/m/d H:i:s', time()); 
                        ...
                        $stmt->bindValue(":data_cadastro", $data_cadastro_aluno);
...
<body class="body-login">
    <section  class="login">
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="form-add">
                <label>Nome</label>
                <input type="text" name="nome" placeholder="Ex.: Ciclano da Silva" class="form-control <?php echo (!empty($nome_err)) ? 'is-invalid' : ''; ?>" autocomplete="off" value="<?php echo $nome; ?>">
                <span class="invalid-feedback"><?php echo $nome_err; ?></span>
            </div>    
            <div class="form-add">
                <label>Avaliação do Aluno</label>
                <input type="text" name="avaliacao" autocomplete="off" class="form-control <?php echo (!empty($avaliacao_err)) ? 'is-invalid' : ''; ?>" placeholder="Ex.: Aluno inteligente porém não esforçado..." value="<?php echo $avaliacao; ?>">
